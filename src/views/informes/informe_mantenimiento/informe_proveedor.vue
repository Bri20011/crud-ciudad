    ...
    <v-container>
        <toolbar flat>
            <v-row>
                ...
                <v-col cols="4">
                    <v-text-field variant="outlined" v-model="filtros.ruc" density="compact"
                        label="Fitro por Ruc" required></v-text-field>
                </v-col>
...
export default {
    ...
    methods: {
        generarReporte(itemsFiltrados) {
            const doc = new jsPDF();
            doc.setFontSize(16);
            doc.text('Reporte de Proveedores', 105, 10, { align: 'center' });
            doc.setFontSize(12);
        ...
        filtrarItems() {
            ...
            if (this.filtros.ruc) {
                items = items.filter(item => item.ruc === this.filtros.ruc)
            }
            return items
        },

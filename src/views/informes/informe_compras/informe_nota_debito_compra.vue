...
export default {
    ...
    methods: {
        generarReporte(itemsFiltrados) {
            const doc = new jsPDF();
            doc.setFontSize(16);
            doc.text('Reporte de Notas de Debitos', 105, 10, { align: 'center' });
            doc.setFontSize(12);

            itemsFiltrados.forEach(item => {
        autoTable(doc, {
            head: [['Codigo', 'Numero de Nota Debito', 'Fecha ND', ' Proveedor']],
        ...
        autoTable(doc, {
            ...
            body: item.itemsDetalle.map(det => [det.nomnbreProducto, det.Cantidad, det.Precio]),
            theme: 'grid', // Agrega bordes a la tabla
            styles: { textColor: [0, 0, 0], fillColor: [255, 255, 255] }, // Color de letra negro y fondo de celda blanco
            columnStyles: { 0: { cellWidth: 30 }, 1: { cellWidth: 'auto' }, 2: { cellWidth: 40 } }
        });
        ...
        async ObtenerNota_Debito_C() {
        await NDCompraApi.getAll().then(({ data }) => {
        ...
        async descargarReporte() {
            await this.ObtenerNota_Debito_C()

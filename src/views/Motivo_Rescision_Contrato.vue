<template>
  <v-dialog max-width="700" v-model="dialogoFormulario" persistent>
    <v-card class="rounded-xl">
      <v-container>
        <h1 class="mb-3">Crear Motivo Rescisión </h1>
        <v-form>
          <v-row>
            <v-col cols="12" sm="12" md="12">
              <v-text-field variant="outlined" label="Descripcion de Motivo Rescisión " v-model="formulario.descripcion"
  ...
  <v-dialog max-width="700" v-model="dialogoFormularioEditar" persistent>
    <v-card class="rounded-xl">
      <v-container>
        <h1 class="mb-3">Editar Motivo Rescisión </h1>
        <v-form>
          <v-row>
  ...
  <v-container>
    <v-row>
      ...
      <v-col cols="12" sm="7" md="7" class="d-flex justify-end align-center">
        Cantidad de Motivo Rescisión : {{ items.length }}
      </v-col>
      ...
      <v-data-table :headers="headers" :items="itemsComputed">
        <template v-slot:top>
          <v-toolbar flat color="white">
            <v-toolbar-title>
              <p class="font-weight-bold">Motivo Rescisión </p>
            </v-toolbar-title>
...
export default {
  ...
  methods:
  {
    ...
    guardarFormulario() {
      ...
      MotivoRescisionAPI.create(
        {
          idmotivo_rescision_contrato: this.formulario.codigo,
    ...
    eliminarMotivoR() {
      if (this.elementoAEliminar) {
        ...
        MotivoRescisionAPI.delete(this.elementoAEliminar.id).then(() => {
          this.ObtenerMotivoRescision();
        });

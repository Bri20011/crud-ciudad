<template>
    <v-dialog max-width="1200" v-model="dialogoFormulario" persistent>
        <v-card class="rounded-xl">
            <v-container>
                <h1 class="mb-3">Registrar Nota de Debito </h1>
                <v-form>
                    <v-row>
                        ...
                        <v-col cols="12" sm="3" md="3">
                            <v-text-field variant="outlined" label="Numero de Nota de Debito"
            ...
            <v-data-table items-per-page-text="Item por pagina" :headers="headers" :items="itemsComputed">
                <template v-slot:top>
                    <v-toolbar flat color="white">
                        <v-toolbar-title>
                            <p class="font-weight-bold">Registro de Notas de Debito</p>
                        </v-toolbar-title>
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { title: 'Numero de Nota de Debito', key: 'numero_nc' },
    ...
    CajaAPI.getAll().then(({ data }) => {
        ...
        guardarFormulario() {
            ...
            NDVentaApi.create({
                idNota_Debito_Venta: this.formulario.codigo,
        ...
        cambiarEstadoCompra() {
            if (this.elementoACambiarEstado) {
                ...
                NDVentaApi.update(this.elementoACambiarEstado.id, { estado_nc: true }
                ).then(() => {
    ...
    ObtenerVentas() {
        ...
        ObtenerNota_Debito_V() {
            NDVentaApi.getAll().then(({ data }) => {
    ...
    async created() {
        ...
        this.ObtenerIva()
        await this.ObtenerNota_Debito_V()
        this.ObtenerCaja()
        this.ObtenerTipoD()
        this.ObtenerVentas()

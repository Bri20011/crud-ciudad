<template>
    ...
    <v-container>
        <v-row>
            ...
            <v-col cols="12" sm="7" md="7" class="d-flex justify-end align-center">
                Cantidad de Cobranza: {{ items.length }}
            </v-col>
            ...
            <v-data-table items-per-page-text="Articulo por pagina" :headers="headers" :items="itemsComputed">
                <template v-slot:top>
                    <v-toolbar flat color="white">
                        <v-toolbar-title>
                            <p class="font-weight-bold">Registrar Cobranza por forma de pago</p>
                        </v-toolbar-title>
    ...
    <v-dialog max-width="700" v-model="dialogoFormularioVistaAprobar" persistent>
        <v-card class="rounded-xl">
            <v-container>
                <h2 class="mb-3">Registrar Cobranza por Forma de Cobro</h2>
                <v-form>

                    <v-row>
                        <v-col cols="12" sm="5" md="5">
                            <v-autocomplete variant="outlined" label="Seleccione Cliente" :items="listaProveedor"
                                item-title="descripcionP" item-value="id" v-model="formulario.proveedor"></v-autocomplete>
                        </v-col>
                        <v-col cols="12" sm="3" md="3">
                            <input class="custom-input" v-model="formulario.fechaD" type="date"
                                placeholder="Fecha de Pago" @input="formatDate" />
                        </v-col>
                        <v-col cols="12" sm="4" md="4">
                            <v-autocomplete variant="outlined" label="Forma de Pago" :items="listaProveedor"
                                item-title="descripcionP" item-value="id" v-model="formulario.proveedor"></v-autocomplete>
                        </v-col>

                        <v-col cols="12" sm="6" md="6 " class="">
                            <v-text-field variant="outlined" label="Descripcion" v-model="formulario.descripciosn"
                                required></v-text-field>
                        </v-col>

                       
                        <v-col cols="12" sm="3" md="3">
                            <v-autocomplete variant="outlined" label="Caja" :items="listaProveedor"
                                item-title="descripcionP" item-value="id" v-model="formulario.proveedor"></v-autocomplete>
                        </v-col>
                        <v-col cols="12" sm="3" md="3 " class="">
                            <v-text-field variant="outlined" label="Monto de Pago" v-model="formulario.descripcions"
                                required></v-text-field>
                        </v-col>
                    </v-row>
    ...
    <presupuesto-c></presupuesto-c>
    <!-- Para importar aqui.. si hay 2 letras uno mayuscula otro minuscula, se puede poner - y seguido la letra siguiente pero ya en minuscula -->
</template>
...
export default {
    components: {
        VDataTable,
        presupuestoC // Para importar aqui, se declara como componente//
    },
    data() {
        return {
            ...
            headers: [
                ...
                { title: 'Monto de Pago', key: 'fechaDd', align: 'star' },
            ...
            headersPedidoApr: [
                ...
                { title: 'Producto', key: 'idProducto' },
                { title: 'Descripcion', key: 'Cantidad', align: 'star' },
                { title: 'Importe', key: 'Precio', align: 'star' },
                { title: 'Monto de Pago', key: 's', align: 'star' },
                { title: 'Accion', key: 'action', sortable: false, align: 'end' },

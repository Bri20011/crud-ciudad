<template>
  <v-dialog max-width="700" v-model="dialogoFormulario" persistent>
    <v-card class="rounded-xl">
      <v-container>
        <h1 class="mb-3">Crear Tipo de Venta</h1>
        <v-form>
          <v-row>
            <v-col cols="12" sm="12" md="12">
              <v-text-field variant="outlined" label="Descripcion de Tipo de Venta" v-model="formulario.descripcion"
  ...
  <v-dialog max-width="700" v-model="dialogoFormularioEditar" persistent>
    <v-card class="rounded-xl">
      <v-container>
        <h1 class="mb-3">Editar Tipo de Venta</h1>
        <v-form>
          <v-row>
  ...
  <v-container>
    <v-row>
      ...
      <v-col cols="12" sm="7" md="7" class="d-flex justify-end align-center">
        Cantidad de Tipo de Venta: {{ items.length }}
      </v-col>
      ...
      <v-data-table :headers="headers" :items="itemsComputed">
        <template v-slot:top>
          <v-toolbar flat color="white">
            <v-toolbar-title>
              <p class="font-weight-bold">Tipo de Ventas</p>
            </v-toolbar-title>
        ...
        <template v-slot:item.action="{ item }">
          <v-icon size="small" class="me-2" @click="editarTipoVenta(item.raw)">
            mdi-pencil
          </v-icon>
          <v-icon color="#C62828" size="small" @click="confirmarEliminarTipoVenta(item.raw)">
    ...
    <v-row>
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-row>
         <v-col cols="12" class="d-flex justify-end">
          <v-btn color="#E0E0E0" class="mx-2" text @click="eliminarTipoVenta">Eliminar</v-btn>
          <v-btn color="primary" text @click="cancelarEliminarTipoVenta">Cancelar</v-btn>
         </v-col>
        </v-row>
...
export default {
  ...
  methods:
  {
    ...
    guardarFormulario() {
      ...
      TipoVentaAPI.create(
        {
          idtipo_venta: this.formulario.codigo,
    ...
    eliminarTipoVenta() {
      if (this.elementoAEliminar) {
        ...
        TipoVentaAPI.delete(this.elementoAEliminar.id).then(() => {
          this.ObtenerTipoVenta();
        });

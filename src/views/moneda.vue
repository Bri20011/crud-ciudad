<template>
        ...
        <v-form>
          <v-row>
            ...
            <v-col cols="12" sm="12" md="12">
              <v-text-field variant="outlined" label="Descripcion de Moneda" v-model="formulario.descripcion"
  ...
  <v-dialog max-width="700" v-model="dialogoFormularioEditar" persistent>
    <v-card class="rounded-xl">
      <v-container>
        <h1 class="mb-3">Editar Moneda</h1>
        <v-form>
          <v-row>
            <v-col cols="12" sm="2" md="2">
  ...
  <v-container>
    <v-row>
      ...
      <v-col cols="12" sm="7" md="7" class="d-flex justify-end align-center">
        Cantidad de Moneda: {{ items.length }}
      </v-col>
...
export default {
  ...
  methods:
  {
    ...
    guardarFormulario() {
      ...
      MonedaAPI.create(
      {
        idmoneda: this.formulario.codigo,
   ...
   confirmarEliminarCiudad(elemento) {
    ...
    eliminarCiudad() {
      if (this.elementoAEliminar) {
        ...
        MonedaAPI.delete(this.elementoAEliminar.id).then(() => {
          this.ObtenerMoneda();
        });

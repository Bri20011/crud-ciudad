<template>
  <v-dialog max-width="700" v-model="dialogoFormulario" persistent>
    <v-card class="rounded-xl">
      <v-container>
        <h1 class="mb-3">Crear Motivo de Traslado </h1>
        <v-form>
          <v-row>
            <v-col cols="12" sm="12" md="12">
              <v-text-field variant="outlined" label="Descripcion de Motivo Traslado " v-model="formulario.descripcion"
  ...
  <v-dialog max-width="700" v-model="dialogoFormularioEditar" persistent>
    <v-card class="rounded-xl">
      <v-container>
        <h1 class="mb-3">Editar Motivo Traslado </h1>
        <v-form>
          <v-row>
  ...
  <v-container>
    <v-row>
      ...
      <v-col cols="12" sm="7" md="7" class="d-flex justify-end align-center">
        Cantidad de Motivo Traslado : {{ items.length }}
      </v-col>
      ...
      <v-data-table :headers="headers" :items="itemsComputed">
        <template v-slot:top>
          <v-toolbar flat color="white">
            <v-toolbar-title>
              <p class="font-weight-bold">Motivo Traslado </p>
            </v-toolbar-title>
...
import { MotivoTrasladoAPI } from '@/services/motivo_traslado_remision.api'
export default {
  components: {
    VDataTable
  },
  ...
  methods:
  {
    ...
    guardarFormulario() {
      ...
      MotivoTrasladoAPI.create(
        {
          idmotivo_traslado_remision: this.formulario.codigo,
          Descripcion: this.formulario.descripcion
        }
      ).then(()=> {
        this.ObtenerMotivoTraslado()
      })
    ...
    eliminarMotivoR() {
      if (this.elementoAEliminar) {
        ...
        MotivoTrasladoAPI.delete(this.elementoAEliminar.id).then(() => {
          this.ObtenerMotivoTraslado();
        });
    ...
    ObtenerMotivoTraslado() {
      MotivoTrasladoAPI.getAll().then(({data}) => {
        this.items = data.map(item=> {
          return {
            id: item.idmotivo_traslado_remision,

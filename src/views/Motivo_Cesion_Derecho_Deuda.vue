<template>
  <v-dialog max-width="700" v-model="dialogoFormulario" persistent>
    <v-card class="rounded-xl">
      <v-container>
        <h1 class="mb-3">Crear Motivo Cesion Derecho y Deudas </h1>
        <v-form>
          <v-row>
            <v-col cols="12" sm="12" md="12">
              <v-text-field variant="outlined" label="Descripcion de Cesion Derecho y Deudas " v-model="formulario.descripcion"
  ...
  <v-dialog max-width="700" v-model="dialogoFormularioEditar" persistent>
    <v-card class="rounded-xl">
      <v-container>
        <h1 class="mb-3">Editar Motivo Cesion Derecho y Deudas </h1>
        <v-form>
          <v-row>
            ...
            <v-col cols="12" sm="10" md="10">
              <v-text-field variant="outlined" label="Descripcion de Motivo Cesion Derecho y Deudas " v-model="formulario.descripcion"
                :error="excededLimit" :error-messages="errorMessage" required></v-text-field>
            </v-col>
          </v-row>
          <v-row>
  ...
  <v-container>
    <v-row>
      ...
      <v-col cols="12" sm="7" md="7" class="d-flex justify-end align-center">
        Cantidad de Motivo Cesion: {{ items.length }}
      </v-col>
      ...
      <v-data-table :headers="headers" :items="itemsComputed">
        <template v-slot:top>
          <v-toolbar flat color="white">
            <v-toolbar-title>
              <p class="font-weight-bold">Motivo Cesion </p>
            </v-toolbar-title>
...
export default {
  ...
  methods:
  {
    ...
    guardarFormulario() {
      ...
      MotivoCesionAPI.create(
        {
          idmotivo_cesion_derecho_deuda: this.formulario.codigo,
          Descripcion: this.formulario.descripcion
        }
      ).then(()=> {
        this.ObtenerMotivoCesionDeuda()
      })
    ...
    eliminarMotivoR() {
      if (this.elementoAEliminar) {
        ...
        MotivoCesionAPI.delete(this.elementoAEliminar.id).then(() => {
          this.ObtenerMotivoCesionDeuda();
        });

<template>
    ...
    <CesionFormulario v-if="dialogoFormulario" @cerrar-dialogo="dialogoFormulario = false" />
</template>
<script>
...
export default {
    ...
    computed: {
        itemsComputed() {
            if (!this.buscador) return this.prop_listado_cesion
            return this.prop_listado_cesion.filter((element) => element.id.toString().toLocaleLowerCase().includes(this.buscador.toLocaleLowerCase()))
        },
    },
    methods: {
        abrirDialogo() {
